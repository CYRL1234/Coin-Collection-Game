...
const gameOverPanel = (function() {
    ...
    const show = function() {
        console.log(gammingpanel.getwinner());
        console.log(gammingpanel.getPlayerId());
        if(gammingpanel.getwinner()==gammingpanel.getPlayerId()){
            $('#result-status').text('Win'); 
            //const users = JSON.parse(fs.readFileSync("./data/users.json"));
        }
        else if(gammingpanel.getwinner()==2){
            $('#result-message').text('Draw');
            $('#result-status').text('Game');
        }
        else{
            $('#result-status').text('Loose');
        }
...
const gammingpanel = (function() {
    ...
    const startGame = function(){
        ...
        let  walls = [BoundingBox(context, 0, 0, 420, 5),
            BoundingBox(context, 200, 10, 210, 110),
            BoundingBox(context, 100, 100, 200, 110),
            BoundingBox(context, 0, 500, 200, 510),
            BoundingBox(context, 190, 300, 200, 500),
            BoundingBox(context, 300, 600, 310, 800),
            BoundingBox(context, 100, 590, 310, 600),
            BoundingBox(context, 300, 300, 420, 310),
            BoundingBox(context, 300, 100, 310, 300),
            BoundingBox(context, 0, 795, 420, 800),
            BoundingBox(context, 0, 0, 5, 800),
            BoundingBox(context, 415, 0, 420, 800),
            ...
            switch (playerId){
                ...
                for (i = 0; i < gems.length; i = i + 1){
                    ...
                    if(intersect == true){
                        ...
                        let currentHP = parseInt($('#HP').text(), 10);
                        currentHP+=1;
                ...
                case 1:
                    ...
                    for (i = 0; i < gems.length; i = i + 1){
                        ...
                        let gem_x = gemPosition.x;
                        let gem_y = gemPosition.y;
                        let intersect = playerBox2.isPointInBox(gem_x,gem_y);
                        gemId = i;
                        //console.log(gemId);
                        if(intersect == true){
                            sounds.collect.currentTime = 0;
    ...
    return { initialize, show, hide, setPlayerId,startGame,setCanPlay,getCanPlay,getPlayerId
        ...
        ,setgemchange,getgemchange,setk,getk,setwinner,getwinner,TP
     };
}

...
io.on('connection', (socket) => {
    ...
    socket.on("endGame", () => {
        if(winner == -1){
            if(players[0].gems>players[1].gems){
                winner = 0;
            }
            else if(players[0].gems<players[1].gems){
                winner = 1;
            }else{
                winner = 2;
            }
        }
        io.emit("winner",JSON.stringify({winner:winner}))
    ...
    socket.on("playerCollectGem", (data) => {
        ...
        io.emit("gem_collected",JSON.stringify({gemId: gemId,k : Math.floor(Math.random() * 10)}))
    })
